...
def cutmix(images, labels, alpha=1.0):
    ...
    return images, labels  # Start with OFF!
...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_filepaths, all_labels), 1):
    ...
    ).shuffle(len(train_idx), seed=SEED).map(process_img).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)

    val_ds = tf.data.Dataset.from_tensor_slices(
        (all_filepaths[val_idx], val_labels)
    ).map(process_img).batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)

    # Optional CutMix
    # train_ds = train_ds.map(lambda x, y: cutmix(x, y))
    ...
    for layer in base_model.layers:
        layer.trainable = False  # Feature extraction phase
