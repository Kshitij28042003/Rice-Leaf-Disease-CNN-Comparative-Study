...
np.random.seed(SEED)
# /mnt/shared/College_work_data/Data_sets
...
for idx, class_name in enumerate(CLASSES):
    ...
    files = glob.glob(os.path.join(aug_path, "*.jpg")) + \
            ...
            glob.glob(os.path.join(aug_path, "*.JPG"))
...
train_ds = train_ds.map(lambda images, labels: cutmix(images, labels))
...
model = load_model("/mnt/shared/College_work_data/Data_sets/CP_MODEL/IN_PHASE1_cutmix.h5")
...
# === Save ===
SAVE_PATH = "/mnt/shared/College_work_data/Data_sets/CP_MODEL/IN_PHASE2_cutmix.h5"
